...
| 模型 | 大小 | 输入 | 定位 |
| --- | --- | --- | --- |
| MODNet FP32 | 25.9MB | 256×256 | 通用平衡（默认，离线可用） |
| MODNet INT8 | 6.6MB | 256×256 | 快速 / 移动端推荐 |
| U²-Net 便携（u2netp） | 4.7MB | 320×320 | 轻量通用，比 MODNet 更稳 |
| ISNet-Anime（imgly 引擎） | ~40MB（CDN 缓存） | 1024×1024 | 动漫立绘最佳，WebGPU 加速（首次需联网） |

模型按路径独立缓存，下拉切换自动重新加载。移动端 CPU 跑 MODNet INT8 足够；Tauri 阶段可升级原生 ONNX Runtime（自动 CoreML/NNAPI 加速）。imgly 引擎基于 `@imgly/background-removal`，通过 CDN ESM 动态加载，首次使用自动下载模型并缓存，离线场景自动降级到本地 MODNet。

## 功能清单

- **上传**：多图拖拽/点选、逐张角度输入、单张删除、一键清空
- **抠图**：ONNX 模型推理（MODNet/U²-Net）/ imgly ISNet 高精度引擎（WebGPU）/ 透明底直通 / 色键去白底，共四种模式
- **合成**：渐变/星空/纯色/自定义渐变背景、**全息·彩虹闪/宇宙闪/反向闪**三种箔面背景、5 种字体、主体缩放、烫金描边
- **3D 查看**：**弹簧物理拖拽惯性**、键盘 ←/→（6°/格）、0/90/180/270 跳转 + 自定义角度（均走弹簧过渡）、自动旋转、任意角度环形插值、双面渲染
- **效果**：光泽扫光、**彩虹箔面色移（随旋转角度变化）**、景深、转速滑块
...
- 3D 旋转采用 spring-damper 物理模型（K=90, D=20，轻微过阻尼无振荡），拖拽松手有自然抛射惯性，角度跳转走最短路径弹簧过渡，替代原线性减速；角速度钳制 ±6rad/s 防止数值发散，低于 0.15rad/s 视为静止点击不给惯性。
- imgly 抠图引擎通过 `import()` 动态加载 CDN ESM 包，不引入打包器；首次使用需联网下载 ISNet 模型，后续浏览器缓存。
- 全息箔面背景为纯 Canvas2D 程序化生成（光栅条纹 + color-dodge/color-burn 混合 + 程序化噪点），无需外部素材；查看器 overlay 层的彩虹箔面高光随旋转角度位移，模拟真实全息卡的色移效应。
